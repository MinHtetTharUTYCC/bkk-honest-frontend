...
import { PriceReportRow, SpotData } from "@/types/spot";
...
export default function PricesTab({ spot }: PricesTabProps) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      {showPriceModal && <CreatePriceModal spotId={spotId} onClose={() => setShowPriceModal(false)} />}
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-border">
              <th className="px-8 py-6 text-xs font-semibold tracking-wide text-white/50">Item Name</th>
              <th className="px-8 py-6 text-xs font-semibold tracking-wide text-white/50">Price</th>
              <th className="px-8 py-6 text-xs font-semibold tracking-wide text-white/50">Status</th>
              <th className="px-8 py-6 text-xs font-semibold tracking-wide text-white/50">Date</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {!Array.isArray(reports) || reports.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-8 py-20 text-center text-sm font-medium text-white/40">
                  No reports yet
                </td>
              </tr>
            ) : (
      ...
      <div ref={pricesObserverTarget} className="py-6 flex justify-center">
        {isFetchingNextReports ? (
          ...
          <p className="text-[10px] font-semibold text-white/40 tracking-wide">End of reports</p>
        )}
      </div>
    </div>
  );

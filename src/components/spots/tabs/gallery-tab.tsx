...
import {
    GalleryControllerGetGallerySort,
...
export default function GalleryTab({ spot }: GalleryTabProps) {
    ...
    const [selectedImage, setSelectedImage] = useState<GalleryImageResponseDto | null>(null);
    const [deleteImageId, setDeleteImageId] = useState<string | null>(null);

    // Sync state with URL params
    const sort =
        (searchParams.get('sort') as GalleryControllerGetGallerySort) ||
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const response = await uploadMutation.mutateAsync({ spotId, data: { image: file } });
            const newImage = response.data as GalleryImageResponseDto;
            ...
            sortTypes.forEach((sortType) => {
                queryClient.setQueryData<InfiniteData<GalleryPage>>(
                    ...
                    (oldData) => {
                        ...
                        if (newPages.length === 0) {
                            ...
                            const pageData = firstPage.data as PaginatedGalleryImagesResponseDto;
                            firstPage.data = {
                                ...pageData,
                                data: [newImage, ...(pageData.data || [])],
                            };
    ...
    const handleSortChange = (newSort: GalleryControllerGetGallerySort) => {
    ...
    const handleFlagImage = async (id: string) => {
        if (!authUser) {
            toast.error('Join us first to report!', {
                description: 'Please login to help keep the community safe.',
            });
            return;
        }
        try {
            ...
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This action cannot be undone. This will permanently remove this photo
                            from the gallery.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
            ...
            {isLoading ? (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 px-2">
                    {[1, 2, 3, 4, 5, 6].map((i) => (
                        <div
                            key={i}
                            className="aspect-4/5 bg-white/5 rounded-2xl animate-pulse border border-white/5"
                        />
                    ))}
                </div>
            ) : images.length === 0 ? (
                    ...
                    {images.map((img) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="p-2.5 md:p-4 bg-white/2">

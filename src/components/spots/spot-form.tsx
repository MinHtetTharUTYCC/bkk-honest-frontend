...
import {
    spotFormValidation,
    SpotFormData as SpotFormValues,
} from '@/lib/validations/spot-form.validation';
import {
    Form,
    FormControl,
    FormField,
...
export default function SpotForm({
    ...
    const form = useForm<SpotFormValues>({
        resolver: zodResolver(spotFormValidation),
        defaultValues: {
        ...
        async (lat: number, lng: number) => {
            ...
            try {
                ...
                if (addr) {
                    ...
                    // If no address; clear or leave as-is. Log for investigation.
                    // eslint-disable-next-line no-console
                    console.warn('Reverse geocode did not return address', response);
                }
            } catch (error) {
        ...
        <Form {...form}>
            <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-8">
                ...
                <div className="space-y-4">
                    ...
                    {errors.image && (
                        <p className="text-xs text-red-400">{String(errors.image.message)}</p>
                    )}
                </div>

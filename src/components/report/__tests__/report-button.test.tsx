...
describe("ReportButton Component", () => {
  ...
  describe("Modal Interaction", () => {
    it("should open report modal when button is clicked", async () => {
      ...
      // Modal should show
      expect(
        screen.getByRole("heading", { name: /report content/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/help us keep the community safe/i),
      ).toBeInTheDocument();
    });
    ...
    it("should show report form with reason select and description textarea", async () => {
      ...
      await user.click(
        screen.getByRole("button", { name: /report this content/i }),
      );
      ...
      expect(
        screen.getByPlaceholderText(/help us understand/i),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Form Submission", () => {
    ...
    it("should close modal after successful submission", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.queryByRole("heading", { name: /report content/i }),
    ...
    it("should close modal when close button is clicked", async () => {
      ...
      const closeButton = buttons.find(
        (btn) =>
          ...
          btn.classList.contains("hover:bg-white/12"),
      );

      if (closeButton && closeButton !== buttons[0]) {
        // Make sure it's not the main report button

            ...
            <ScrollArea className="w-full whitespace-nowrap -mx-8 px-8">
                <div className="flex gap-4 pb-4">
                    {[1, 2, 3].map((i) => (
                        <div
                            key={i}
                            className="shrink-0 min-w-50 h-20 bg-white/5 rounded-2xl animate-pulse"
                        />
                    ))}
                </div>
            ...
            <div className="flex gap-4 pb-4">
                {list.map((c, idx: number) => (
                    <Link
                        key={c.id}
                        href={`/profile/${c.id}`}
                        className="shrink-0 bg-card p-5 rounded-2xl border border-white/8 shadow-xl shadow-black/30 flex items-center gap-4 min-w-50 hover:border-amber-400/50 transition-colors"
                    >
                        <div
                            className={cn(

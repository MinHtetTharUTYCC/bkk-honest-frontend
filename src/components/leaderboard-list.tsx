        ...
        <div className="space-y-5">
            {list.map((c: any, idx: number) => (
                <div key={c.id} className="flex items-center gap-3">
                    <div className={cn('w-10 h-10 rounded-2xl flex items-center justify-center font-black text-xs shadow-sm overflow-hidden', LEVEL_COLORS[idx % LEVEL_COLORS.length])}>
                        {c.avatarUrl
                    ...
                    <div className="flex flex-col">
                        <span className="text-sm font-semibold text-foreground leading-tight truncate max-w-[160px]">{c.name || 'Anonymous'}</span>

...
export function ProfileTabs({
  ...
  const reportsPages = reportsData?.pages as
    | InfinitePage<ReportItem>[]
    | undefined;
  ...
  const reportsList = useMemo(
    () => reportsPages?.flatMap((page) => page.data || []) || [],
    [reportsPages],
  );
  const scamsList = useMemo(
    () => scamsPages?.flatMap((page) => page.data || []) || [],
    [scamsPages],
  );
  const tipsList = useMemo(
    () => tipsPages?.flatMap((page) => page.data || []) || [],
    [tipsPages],
  );
  const spotsList = useMemo(
    () => spotsPages?.flatMap((page) => page.data || []) || [],
    [spotsPages],
  );

  const reportsTotal =
    reportsPages?.[0]?.pagination?.total || reportsList.length;
  ...
  const tabs = [
    {
      ...
      count: scamsTotal,
      color:
        "text-red-400 bg-red-500/10 border-red-500/20 active:bg-red-500/20",
    },
    {
      id: "tips",
      label: "Tips",
      count: tipsTotal,
      color:
        "text-emerald-400 bg-emerald-500/10 border-emerald-500/20 active:bg-emerald-500/20",
    },
    {
      id: "reports",
      label: "Prices",
      count: reportsTotal,
      color:
        "text-amber-400 bg-amber-500/10 border-amber-500/20 active:bg-amber-500/20",
      private: true,
    },
    {
      id: "spots",
      ...
      count: spotsTotal,
      color:
        "text-orange-400 bg-orange-500/10 border-orange-500/20 active:bg-orange-500/20",
    },
  ] as const;

  const visibleTabs = tabs.filter(
    (tab) => !isPublic || !("private" in tab && tab.private),
  );
      ...
      <div className="space-y-6">
          ...
          <div className="flex flex-col gap-6">
            {scamsList.length > 0 ? (
              scamsList.map((scam) => (
                <div
                  key={scam.id}
                  className="bg-card rounded-2xl border border-white/8 shadow-xl shadow-black/20 border-l-4 border-l-red-500 overflow-hidden flex flex-col sm:flex-row items-stretch"
                >
                  <div className="w-full sm:w-32 md:w-40 aspect-square bg-white/5 border-b sm:border-b-0 sm:border-r border-white/10 overflow-hidden shrink-0 flex items-center justify-center relative">
                    {scam.imageVariants &&
                    Object.values(scam.imageVariants).some((v) => v) ? (
                      <Image
                        src={
                          scam.imageVariants.display ||
                          scam.imageVariants.thumbnail ||
                          ""
                        }
                        alt={scam.scamName}
                        fill
            ...
            {tipsList.length > 0 ? (
              tipsList.map((tip) => {
                const displayVariants =
                  tip.imageVariants &&
                  Object.values(tip.imageVariants).some((v) => v)
                    ? tip.imageVariants
                    : tip.spot?.imageVariants;
                return (
                  <div
                    key={tip.id}
                    className="bg-card rounded-2xl border border-white/8 shadow-xl shadow-black/20 border-l-4 border-l-emerald-400 overflow-hidden flex flex-col sm:flex-row items-stretch"
                  >
                    <div className="w-full sm:w-32 md:w-40 aspect-square bg-white/5 border-b sm:border-b-0 sm:border-r border-white/10 overflow-hidden shrink-0 flex items-center justify-center relative">
                      {displayVariants &&
                      Object.values(displayVariants).some((v) => v) ? (
                        <Image
                          src={
                            displayVariants.display ||
                            displayVariants.thumbnail ||
                            ""
                          }
                          alt={tip.title}
                          fill
          ...
          <div className="flex flex-col gap-6">
            {reportsList.length > 0 ? (
              reportsList.map((report) => {
                const displayVariants =
                  report.imageVariants &&
                  Object.values(report.imageVariants).some((v) => v)
                    ? report.imageVariants
                    : report.spot?.imageVariants;
                return (
                  <div
                    key={report.id}
                    className="bg-card rounded-2xl border border-white/8 shadow-xl shadow-black/20 border-l-4 border-l-amber-400 overflow-hidden flex flex-col sm:flex-row items-stretch"
                  >
                    <div className="w-full sm:w-32 md:w-40 aspect-square bg-white/5 border-b sm:border-b-0 sm:border-r border-white/10 overflow-hidden shrink-0 flex items-center justify-center relative">
                      {displayVariants &&
                      Object.values(displayVariants).some((v) => v) ? (
          ...
          <div className="flex flex-col gap-6">
            {spotsList.length > 0 ? (
              spotsList.map((spot) => (
                <div
                  key={spot.id}
                  className="bg-card rounded-2xl border border-white/8 shadow-xl shadow-black/20 border-l-4 border-l-orange-400 overflow-hidden flex flex-col sm:flex-row items-stretch"
                >
                  <div className="w-full sm:w-32 md:w-40 aspect-square bg-white/5 border-b sm:border-b-0 sm:border-r border-white/10 overflow-hidden shrink-0 flex items-center justify-center relative">
                    {spot.imageVariants &&
                    Object.values(spot.imageVariants).some((v) => v) ? (
                      <Image
                        src={
                          spot.imageVariants.display ||
                          spot.imageVariants.thumbnail ||
                          ""
                        }
                        alt={spot.name}
                        fill
        ...
        {(() => {
          const shouldShowTrigger =
            ...
            (activeTab === "reports" &&
              (hasNextReports || isFetchingReports)) ||

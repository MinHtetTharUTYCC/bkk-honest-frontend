      ...
      {scamsLoading ? (
        ...
        <div className="space-y-2.5 pb-8">
          {scams.map((scam) => (
            <Link
              key={scam.id}
              href={getScamAlertUrl(scam.slug || scam.id, scam.city?.slug || "")}
            ...
            >
              <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-sm text-red-400 line-clamp-2">{scam.scamName}</h3>
                  <p className="text-xs text-white/60 line-clamp-2 mt-1">{scam.description}</p>
          ...
          <div ref={observerTarget} className="py-6 flex justify-center">
            {isFetchingNextScams ? (
              ...
              <p className="text-[10px] font-semibold text-white/40 tracking-wide">End of scam alerts</p>
            )}
          </div>
        </div>
      )}

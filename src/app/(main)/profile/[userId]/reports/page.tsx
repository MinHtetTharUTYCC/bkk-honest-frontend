...
import { priceReportsControllerFindByUser } from "@/api/generated/price-reports/price-reports";
...
export default async function UserReportsPage({
  ...
  await queryClient.prefetchInfiniteQuery({
    queryKey: ["user-reports-infinite", userId],
    initialPageParam: 0,
    queryFn: async ({ pageParam = 0 }) => {
      const res = await priceReportsControllerFindByUser(userId, {
  ...
  return (
    <HydrationBoundary state={dehydrate(queryClient)}>
      <UserReportsInfiniteTab userId={userId} />
    </HydrationBoundary>
  );
}

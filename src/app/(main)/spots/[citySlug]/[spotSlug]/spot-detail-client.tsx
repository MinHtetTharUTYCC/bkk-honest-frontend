...
export default function SpotDetailClient() {
  ...
  const urlTab = searchParams.get("tabs") as
    ...
    | null;
  const initialTab =
    urlTab && ["gallery", "prices", "tips", "vibes"].includes(urlTab)
      ? urlTab
      : "tips";
  const [activeTab, setActiveTabState] = useState<
    "gallery" | "prices" | "tips" | "vibes"
  >(initialTab);

  useEffect(() => {
    if (
      urlTab &&
      ["gallery", "prices", "tips", "vibes"].includes(urlTab) &&
      urlTab !== activeTab
    ) {
  ...
  const { toggleVote: toggleTipVote, isPending: tipVotePending } =
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              spot and all associated contributions (tips, photos, vibe checks).
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deleteSpotMutation.isPending}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={(e) => {
      ...
      <header className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div
            ...
            onClick={() =>
              spot.imageUrl ? setShowImageViewer(true) : undefined
            }
          >
            {spot.imageUrl ? (
              <img
                    ...
                    >
                      <div className="w-4 h-4 rounded-full bg-white/10 overflow-hidden shrink-0 group-hover/author:border-amber-400 transition-colors">
                        <img
                          src={
                            spot.user.avatarUrl ||
                            `https://api.dicebear.com/7.x/avataaars/svg?seed=${spot.user.id}`
                          }
                          alt={spot.user.name}
                      ...
                      <span className="text-[10px] text-white/40 font-semibold tracking-wide">
                        Discovered by{" "}
                        <span className="text-white/70 group-hover/author:text-amber-400 transition-colors">
                          @{spot.user.name}
                        </span>
                      </span>
                    </Link>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {spot.user && (
                      <Link
                        href={`/profile/${spot.user.id}`}
                        className="flex items-center gap-2 pl-[26px] group/author"
                      >
                        ...
                        <span className="text-xs text-white/40 font-semibold tracking-wide">
                          Discovered by{" "}
                          <span className="text-white/80 group-hover/author:text-amber-400 transition-colors cursor-pointer">
                            @{spot.user.name}
                          </span>
                        </span>
                      </Link>
                ...
                >
                  ...
                  <div className="absolute top-3 right-3 flex bg-white/10 p-0.5 rounded-lg border border-border shadow-sm">
                    <LikeButton
                      ...
                      title={
                        img.hasVoted ? "Unlike this image" : "Like this image"
                      }
                    />
                  </div>
                      ...
                      <td className="px-8 py-6">
                        <div
                          className={cn(
                            ...
                            Number(r.priceThb) <= (spot.priceStats as any).avg

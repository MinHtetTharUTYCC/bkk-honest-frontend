...
function NavigatePageContent() {
  ...
  const getExternalMapUrl = (
    ...
    if (isIOS) {
      // Apple Maps link
      const origin =
        startLat && startLng ? `&saddr=${startLat},${startLng}` : "";
      return `maps://?daddr=${destLat},${destLng}${origin}`;
    }

    // Google Maps Universal Link
    const origin =
      startLat && startLng ? `&origin=${startLat},${startLng}` : "";
      ...
      <div className="absolute top-0 left-0 right-0 bg-linear-to-b from-gray-900 to-transparent p-4 z-20">
        <div className="flex items-center justify-between">
          ...
          <div className="text-center flex-1 min-w-0">
            <h2 className="text-white font-semibold truncate px-2">
              {destName}
            </h2>
            {route && !isLoading && (
              ...
              {route.steps && route.steps.length > 0 ? (
                ...
                <div className="py-10 text-center space-y-2">
                  ...
                  <p className="text-white/40 text-xs">
                    Follow the path on the map for this short route.
                  </p>
                </div>
              )}
            </div>
      ...
      {isLoading && (
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center z-40">
          <div className="text-center space-y-4">
            <Loader2
              size={32}
              className="text-amber-400 animate-spin mx-auto"
            />

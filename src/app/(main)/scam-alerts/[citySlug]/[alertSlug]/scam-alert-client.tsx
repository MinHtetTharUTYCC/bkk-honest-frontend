...
export default function ScamAlertClient({ alert: initialAlert }: { alert: ScamAlertResponseDto }) {
    ...
    const { data: alertData = initialAlert } = useScamAlertBySlug(
        citySlug === 'thailand' ? '' : citySlug,
        alertSlug,
    );

    const alert = (alertData || initialAlert) as ScamAlert;
                    ...
                    if (newPages.length === 0) {
                        ...
                        if (typeof firstPage === 'object' && firstPage !== null) {
                            (firstPage as Record<string, unknown>).data = [
                                newCommentData,
                                ...(((firstPage as Record<string, unknown>).data as unknown[]) ||
                                    []),
                            ];
                        }
    ...
    const handleEditSubmit = async (commentId: string) => {
        ...
        try {
            ...
            // Response is now the DTO directly (mutator unwraps the envelope)
            const updatedComment = response;
            const commentText =
                (updatedComment as unknown as Record<string, unknown>)?.text || editContent;
                            ...
                            return {
                                ...
                                data: (p.data as unknown[])?.map((comment: unknown) => {
                                    ...
                                    return c.id === commentId
                                        ? { ...c, ...updatedComment, text: commentText }
                                        : comment;
                                }),
                            };
                        }),
    ...
    const handleVoteToggle = async () => {
        ...
        await toggleVote({
            ...
            voteId: alert.voteId ? (typeof alert.voteId === 'string' ? alert.voteId : null) : null,
        });
    };
    ...
    return (
        <div className="min-h-screen bg-background pb-20">
            ...
            <AlertDialog open={isDeleteAlertOpen} onOpenChange={setIsDeleteAlertOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Are you sure you want to delete this scam alert? This action cannot be
                            undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                ...
                {alert.imageVariants && Object.values(alert.imageVariants).some((v) => v) && (
                    ...
                    >
                        <Image
                            ...
                            alt={alert.scamName || 'Scam alert image'}
                            fill
                ...
                <div className="space-y-6">
                                ...
                                >
                                    ...
                                    <DropdownMenuItem
                                        onClick={() => {
                                            if (navigator.share) {
                                                navigator.share({
                                                    title: alert.scamName,
                                                    text: `Watch out for the ${alert.scamName} scam in ${alert.city?.name || 'Thailand'}! ⚠️`,
                    ...
                    <div className="flex items-center justify-between gap-4 pt-4 border-t border-white/8">
                        <div
                            ...
                            onClick={() => router.push(`/profile/${alert.user?.id}`)}
                        >
                            <div className="relative w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center text-white/60 overflow-hidden border border-white/10 group-hover/user:border-amber-400 transition-colors">
                                {alert.user?.avatarUrl ? (
                                    <Image
                                        ...
                                        alt={alert.user?.name || 'User Avatar'}
                                        fill
                                        sizes="40px"
                            ...
                            <div className="flex flex-col">
                                ...
                                {alert.user?.level && (
                                    <span className="text-[8px] font-bold text-amber-400/60 border border-amber-400/10 px-1 py-0.5 rounded uppercase tracking-tighter">
                                        {(alert.user.level as string).replace('_', ' ')}
                                    </span>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="min-w-30">
                            <LikeButton
                                ...
                                title={alert.hasVoted ? 'Remove upvote' : 'Upvote this alert'}
                            />
                        </div>
                    </div>
                </div>
            ...
            {/* Edit Modal */}
            {isEditModalOpen && (
                <ScamEditModal alert={alert} onClose={() => setIsEditModalOpen(false)} />
            )}
            ...
            {alert.imageVariants && Object.values(alert.imageVariants).some((v) => v) && (
                <ImageViewer
                    ...
                    imageVariants={alert.imageVariants}
                    alt={alert.scamName}

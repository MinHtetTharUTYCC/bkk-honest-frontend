...
export async function generateStaticParams() {
    ...
    try {
        const response = await fetch(`${baseUrl}/scam-alerts?take=20`, { next: { revalidate: 3600 } });
...
export default async function ScamAlertPage({
    ...
    await queryClient.prefetchQuery({
        ...
        queryFn: async () => {
            const baseUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000';
            const res = await fetch(`${baseUrl}/scam-alerts/by-slug/${encodeURIComponent(citySlug)}/${encodeURIComponent(alertSlug)}`, {

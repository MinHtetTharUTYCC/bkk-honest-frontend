...
function MapPageContent() {
  ...
  const initialCenter = hasUrlPosition
    ? {
        latitude: urlLat,
        longitude: urlLng,
        zoom: isNaN(urlZoom) ? DEFAULT_CENTER.zoom : urlZoom,
      }
    : DEFAULT_CENTER;
  ...
  const handleMoveEnd = (evt: { viewState: ViewState }) => {
    ...
    if (nearMeActive && userLocation) {
      const dist =
        Math.abs(lat - userLocation.lat) + Math.abs(lng - userLocation.lng);
  ...
  useEffect(() => {
    if (!shouldAutoFit.current || spotsFetching || !spots || !mapRef.current)
      return;
  ...
  const handleNavigate = () => {
    ...
    router.push(
      `/navigate?lat=${selectedSpot.latitude}&lng=${selectedSpot.longitude}&name=${encodeURIComponent(selectedSpot.name)}`,
    );
  };
  ...
  const getCategoryConfig = (slug: string) => {
    return {
      colors:
        CATEGORY_COLORS[slug?.toLowerCase()] || CATEGORY_COLORS["default"],
  ...
  return (
    <div className="fixed top-16 md:top-20 left-0 md:left-20 right-0 bottom-0 overflow-hidden bg-black">
              ...
              >
                ...
                <div
                  className={cn(
                    ...
                    isSelected
                      ? "shadow-[0_0_20px_var(--color-amber-400)] border-white"
                      : "",
                  )}
                >
                  {config.icon}
                ...
                <div className="w-0 h-0 border-l-[6px] border-r-[6px] border-t-8 border-l-transparent border-r-transparent border-t-amber-500 -mt-px" />
              </div>
            </Marker>
          );
        })}
      ...
      <AnimatePresence>
        {viewState.latitude >= 13.4 &&
          viewState.latitude <= 14.2 &&
          viewState.longitude >= 100.2 &&
          viewState.longitude <= 101.0 &&
          viewState.zoom >= 10 && (
            <motion.button
              ...
              <div className="flex gap-3">
                <button
                  onClick={() =>
                    router.push(
                      getSpotUrl(
                        selectedSpot?.city?.slug || "bangkok",
                        selectedSpot?.slug || "",
                      ),
                    )
                  }

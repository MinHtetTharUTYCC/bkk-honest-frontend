...
import { Zap, AlertCircle, MessageCircle, ArrowRight, MapPin, TrendingUp, Navigation, Heart, Hash, Trophy } from 'lucide-react';
          ...
          <ScrollArea className="w-full whitespace-nowrap -mx-8 px-8">
            <div className="flex gap-6 pb-8">
              {nearbyLoading ? (
                ...
                <div className="py-20 text-center w-full bg-gray-50 rounded-[40px] border border-dashed border-gray-300">
          ...
          <div className="space-y-6">
            {scamAlertsLoading ? (
              ...
              scamAlerts.map((alert: any) => (
                <div key={alert.id} className="bg-white rounded-3xl p-8 border border-gray-300 shadow-xl shadow-gray-200/20 group hover:scale-[1.01] transition-transform">
                  ...
                  <div className="flex items-center gap-4 border-t border-gray-300 pt-6">
                    <div className="flex bg-gray-50 p-1 rounded-xl border border-gray-300">
                      <button 
               ...
               <div className="py-20 text-center bg-gray-50 rounded-[40px] border border-dashed border-gray-300">
            ...
            {vibesLoading ? (
              ...
              vibes.slice(0, 3).map((vibe: any) => (
                <div key={vibe.id} className="bg-white rounded-3xl p-8 border border-gray-300 shadow-xl shadow-gray-200/20 group hover:scale-[1.01] transition-transform">
                  <div className="flex items-start justify-between mb-4">
        ...
        <div className="lg:hidden space-y-12 pb-12">
          ...
          <section className="space-y-6">
            <header className="px-2">
              <h4 className="text-xl font-black text-gray-900 tracking-tighter uppercase italic flex items-center gap-2">
              ...
              <div className="flex gap-4 pb-4">
                {hashtags.map(({ tag, count }) => (
                  <Link 
                    key={tag} 
                    href={`/search?q=${tag}`} 
                    className="flex-shrink-0 bg-white px-6 py-4 rounded-3xl border border-gray-300 shadow-xl shadow-gray-200/20 flex flex-col gap-1 active:scale-95 transition-transform"
                  >
                    <span className="text-sm font-black text-gray-900">#{tag}</span>
                    <span className="text-[10px] font-black text-cyan-400 uppercase tracking-widest">{count} reports</span>
                  </Link>
                ))}
              </div>
              <ScrollBar orientation="horizontal" className="hidden" />
            ...
            <ScrollArea className="w-full whitespace-nowrap -mx-8 px-8">
              <div className="flex gap-4 pb-4">
                {topContributors.map((c) => (
                  <div 
                    key={c.name} 
                    className="flex-shrink-0 bg-white p-5 rounded-3xl border border-gray-300 shadow-xl shadow-gray-200/20 flex items-center gap-4 min-w-[200px]"
                  >
                    <div className={cn("w-10 h-10 rounded-2xl flex items-center justify-center font-black text-xs shadow-sm", c.color)}>
                      {c.name.charAt(1).toUpperCase()}
                    </div>
                    <div className="flex flex-col">
                      <span className="text-xs font-black text-gray-900">{c.name}</span>
                      <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">{c.rep} XP</span>
                    </div>
                  </div>
                ))}
              </div>

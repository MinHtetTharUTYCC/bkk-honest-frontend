...
export function generateSpotSchema(spot: SpotSchemaInput, url: string) {
  const spotSchema: Record<string, unknown> = {
    ...
    description:
      spot.description ||
      `Explore local prices, vibes, and tips for ${spot.name || "this spot"} on BKK Honest.`,
    url,
    image: spot.imageVariants?.display
...
export function generateScamAlertSchema(
  alert: ScamAlertSchemaInput,
  url: string,
) {
  return {
    "@context": "https://schema.org",
    ...
    description:
      alert.description ||
      `Learn how to identify and avoid ${alert.scamName || "this scam"} in Bangkok.`,
    image: alert.imageVariants?.display
      ? [alert.imageVariants.display]
      : undefined,

            ...
            mutation.mutate(
                { data: { spotId } },
                {
                    onSuccess: (data) => {
                        ...
                        queryClient.setQueriesData({ predicate: spotPredicate }, (old: unknown) =>
                            updateItemInCache<{
                                id: string;
                                isInMission?: boolean;
...
export function useDeleteMission() {
    ...
    return {
        ...mutation,
        mutate: (id: string, currentStatus = 'all', currentSort = 'newest', spotId?: string) =>
            mutation.mutate(
                { id },
                {
                    onSuccess: () => {
                        ...
                        queryClient.setQueryData(currentKey, (old: unknown) => {
                            ...
                            if (!deletedSpotId) {
                                for (const page of cacheData.pages) {
                                    ...
                                    if (found) {
                                        deletedSpotId = (found.spot as { id?: string } | undefined)
                                            ?.id;
                                        break;
                                    }
                                }

...
export function useMapTransitVisible() {
  ...
  const transitVisible = useMapStore(
    (state: MapStoreState) => state.transitVisible,
  );
  const toggleTransitVisible = useMapStore(
    (state: MapStoreState) => state.toggleTransitVisible,
  );

...
export function useVoteToggle(
  ...
  const applyUpdate = (
    ...
    if ("pages" in old && Array.isArray(old.pages)) {
      return {
        ...old,
        pages: (
          old.pages as Array<PaginatedPage<Voteable> | { data?: Voteable[] }>
        ).map((page) => {
          if (!page || typeof page !== "object") return page;
          const pageData = page as
            | PaginatedPage<Voteable>
            | { data?: Voteable[] };
  ...
  const toggleVote = async (
    ...
    queryClient.setQueriesData<CacheFormat | CacheFormat[]>(
      { predicate },
      (old) => applyUpdate(old, item),
    );
    ...
    try {
      if (item.hasVoted && currentVoteId && currentVoteId !== "temp-id") {
        ...
        const newVoteId =
          voteResponse.data?.voteId ?? voteResponse.voteId ?? null;
        queryClient.setQueriesData<CacheFormat | CacheFormat[]>(
          { predicate },
          (old) =>
